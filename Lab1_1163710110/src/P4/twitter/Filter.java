...
public class Filter {
    ...
    public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
        ...
        List<Tweet> nameTweet = new ArrayList<Tweet>();
        for (int i = 0; i < tweets.size(); i++) {
            if (tweets.get(i).getAuthor().equals(username)) {
                nameTweet.add(tweets.get(i));
            }
        }
        return nameTweet;
    }
    ...
    public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
        // throw new RuntimeException("not implemented");
        List<Tweet> timeTweet = new ArrayList<Tweet>();
        Instant start = timespan.getStart();
        Instant end = timespan.getEnd();
        for (int i = 0; i < tweets.size(); i++) {
            if (!tweets.get(i).getTimestamp().isAfter(end) && !tweets.get(i).getTimestamp().isBefore(start)) {
                timeTweet.add(tweets.get(i));
            }
        }
        return timeTweet;
    }
